  ...
  <div class="space-y-3 md:hidden">
    ...
    >
      ...
      <div class="flex flex-wrap items-start justify-between gap-2 px-4 py-3">
        <div class="flex shrink-0 items-center gap-2.5">
          ...
          <div>
            ...
            <p class="whitespace-nowrap text-heading font-bold">{{ method.label }}</p>
          </div>
        </div>
      ...
      <div class="border-t border-border/60">
        <div class="grid grid-cols-1 gap-1 border-b border-border/40 px-4 py-2.5 sm:grid-cols-[5.5rem_1fr]">
          <span class="text-caption text-muted-foreground">월 납입금</span>
          <span class="font-semibold tabular-nums sm:text-right">{{ formatWon(method.monthlyPayment) }}</span>
        </div>
        <div class="grid grid-cols-1 gap-1 border-b border-border/40 px-4 py-2.5 sm:grid-cols-[5.5rem_1fr]">
        ...
        <div
          ...
          class="grid grid-cols-1 gap-1 border-b border-border/40 px-4 py-2.5 sm:grid-cols-[5.5rem_1fr]"
        >
          <span class="text-caption text-muted-foreground">잔존가치</span>
          <span class="text-caption font-semibold tabular-nums sm:text-right">
            {{ formatWon(method.residualValue) }}
          </span>
        </div>
        <div class="grid grid-cols-1 gap-1 px-4 py-2.5 sm:grid-cols-[5.5rem_1fr]">
          <span class="text-caption text-muted-foreground">소유권</span>
          <span class="text-caption font-semibold sm:text-right">{{ method.ownership }}</span>
        </div>
