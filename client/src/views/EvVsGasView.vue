...
// 상세 비교 막대는 결과 칸(반폭)에 두면 결과가 입력보다 313px 길어져 왼쪽 칸이 빈다(1440px 실측) —
// EvAnnualCostComparison에서 분리해 1×2 아래 전폭으로 내린다.
const costItems = computed(() => [
  {
    key: "gas",
    label: "내연기관",
    value: result.value.gasTotal,
    detail: `연료비 ${formatWon(result.value.gasFuel)} 포함`,
    highlight: result.value.winner === "gas",
  },
  {
    key: "ev",
    label: "전기차",
    value: result.value.evTotal,
    detail: `충전비 ${formatWon(result.value.evFuel)} 포함`,
    highlight: result.value.winner === "ev",
  },
]);
...
<template>
  ...
  <div class="sh-container sh-container--tool space-y-5 py-5">
    ...
    <!-- ===== 전기차 보조금 계산기: EV vs Gas와는 별개 계산기라 독립된 1×2 틀을 쓴다 ===== -->
    <ShCalculatorSplit>
      <template #input>
